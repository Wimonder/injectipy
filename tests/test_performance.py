...
def test_large_number_of_registrations(store: InjectipyStore):
...
def test_deep_resolver_chain_performance(store: InjectipyStore):
    ...
    store.register_resolver("resolver_0", resolver_0)

    # Create a chain using Inject dependencies (more reliable)
    for i in range(1, chain_length):
        ...
        store.register_resolver(f"resolver_{i}", chain_resolver)

    # Time the resolution of the final resolver
    start_time = time.time()
    result = store[f"resolver_{chain_length - 1}"]
    end_time = time.time()
...
def test_evaluate_once_performance(store: InjectipyStore):
    ...
    store.register_resolver("expensive", expensive_resolver, evaluate_once=True)

    # First call
    start_time = time.time()
    result1 = store["expensive"]
    first_call_time = time.time() - start_time

    # Subsequent calls should be much faster
    start_time = time.time()
    result2 = store["expensive"]
...
def test_concurrent_resolver_performance():
    ...
    for i in range(50):
        store.register_resolver(f"cpu_task_{i}", cpu_bound_resolver(i))
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for _ in range(4):  # 4 rounds
            for i in range(50):  # 50 different resolvers
                future = executor.submit(lambda key=f"cpu_task_{i}": store[key])
...
def test_memory_usage_with_cached_resolvers(store: InjectipyStore):
    ...
    for i in range(100):
        store.register_resolver(f"object_{i}", object_creator(i * 100), evaluate_once=True)

    # Access all resolvers to populate cache
    results = []
    for i in range(100):
        result = store[f"object_{i}"]
        results.append(result)

    # Verify all objects are cached (same instances)
    for i in range(100):
        assert store[f"object_{i}"] is results[i]

    # Verify objects have expected data
    for i, result in enumerate(results):
        # The id should match the size parameter (i * 100)
        expected_size = i * 100
...
def test_injection_performance_with_many_parameters():
    ...
    for i in range(50):
        store.register_value(f"dep_{i}", f"value_{i}")
...
def test_stress_test_mixed_operations():
    ...
    for i in range(100):
        store.register_value(f"base_{i}", f"base_value_{i}")
    ...
    for i in range(100):
        ...
        store.register_resolver(f"resolver_{i}", stress_resolver)
...
def test_singleton_creation_performance():
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        ...
        stores = [future.result() for future in as_completed(futures)]

    end_time = time.time()

    # All should be the same instance
    first_store = stores[0]
    assert all(store is first_store for store in stores)
...
def test_resolver_caching_consistency_under_load():
    ...
    def tracked_resolver():
        nonlocal call_count
        # Note: We can't use threading.Lock here as it would need to be imported
        # This is a simplified version for testing
        call_count += 1
        return f"result_{call_count}"

    store.register_resolver("tracked", tracked_resolver, evaluate_once=True)
