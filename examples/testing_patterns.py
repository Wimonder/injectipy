...
from injectipy import inject, Inject, InjectipyStore
...
class TestUserServiceWithIsolatedStore:
    ...
    def create_user_service(self) -> UserService:
        ...
        return UserService(
            email_service=self.test_store[EmailServiceProtocol],
            database=self.test_store[DatabaseProtocol]
        )
...
def test_integration_with_real_dependencies():
    ...
    service = UserService(
        email_service=integration_store[EmailServiceProtocol],
        database=integration_store[DatabaseProtocol]
    )
...
def test_with_fixtures(test_store, mock_email_service, mock_database):
    ...
    service = UserService(
        email_service=test_store[EmailServiceProtocol],
        database=test_store[DatabaseProtocol]
    )
