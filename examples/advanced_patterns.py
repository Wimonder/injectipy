...
def setup_email_services():
    ...
    def create_smtp_service(
        host: str = Inject["smtp_host"],
        port: int = Inject["smtp_port"]
...
class UserRepository:
    ...
    def find_user(self, user_id: int) -> Dict[str, Any]:
        results = self.db.execute(f"SELECT * FROM users WHERE id = {user_id}")
...
def setup_cache():
    ...
    def create_redis_cache(
        host: str = Inject["redis_host"],
        port: int = Inject["redis_port"]
