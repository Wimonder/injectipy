...
class ReleaseManager:
    ...
    def run_command(self, command: list, check: bool = True) -> subprocess.CompletedProcess:
        ...
        return subprocess.run(command, cwd=self.project_root, check=check, 
    ...
    def update_version(self, new_version: str) -> None:
        ...
        if not self.dry_run:
            ...
            if version_script.exists():
                ...
                subprocess.run([sys.executable, str(version_script), "current"], check=True)
    ...
    def release(self, bump_type: Optional[str] = None, target_version: Optional[str] = None) -> None:
        ...
        if not self.dry_run:
            ...
            print(f"   Changes: Version bump, changelog update")
            print()
            ...
            if input("Push release to remote and trigger GitHub Actions? (y/N): ").lower() == 'y':
                ...
                print(f"📖 Monitor the GitHub Actions workflow at:")
                print(f"   https://github.com/Wimonder/injectipy/actions")
                print(f"📦 Release will be available at:")
